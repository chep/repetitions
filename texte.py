            ...
            if self.mots_[index].strLow() not in ignore:
                ...
                while index + pos < len(self.mots_) and nbLettres < maxLoin:
                    if (self.mots_[index + pos].strLow() not in ignore
                            and self.mots_[index + pos].match(
                                self.mots_[index], nbLettresCommunes,
                                comparaison)):
                        self.mots_[index + pos].marque(nbLettres, maxProche,
                                                       maxLoin)
                        self.mots_[index].marque(nbLettres, maxProche, maxLoin)
